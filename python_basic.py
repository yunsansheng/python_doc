#内容来源 http://www.code123.cc/1049.html 分分钟学会一门语言之Python篇
#add by one 是自己后续加的内容..
...
###############################
####add by one;   py2 and py3 encode decode
###############################
8个比特（bit）作为一个字节（byte）
#所以一个字节能表示的最大的整数就是255（二进制11111111=十进制255）
ascii 一个字节， unicode 通常两个字节 utf8 可变长度编码
...
py3中符串是以Unicode编码的
py2中默认是ascii编码的，如果u'AB'就是 unicode编码。
py2 unicode=>utf8                 utf8=>unicode
    #u'中文'.encode('utf-8')     #'abc'.decode('utf-8')
在py2中想要字符串以utf-8编码，除了 coding:utf8 中文必须加u''

py3中 默认str都是unicode编码  类型是str 内存中以unicode表示，不再有unicode类型。
保存到磁盘时，就需要把str变为以字节为单位的bytes
x = b'ABC' bypes类型
bytes的每个字符都只占用一个字节，所以只可以是assci, b"中文"会报错。Unicode表示的str通过encode()方法可以编码为指定的bytes
'中文'.encode('utf-8')
'中文'.encode('ascii')#报错
...
py2 网络硬盘读取特定的格式的str  转成unicode.
py3 网络或磁盘上读取了字节流，那么读到的数据就是bytes,用decode转化成str
...
py3
encode:str to bytes ##这里的str都是unicode 
decode:byets to str 
#######################end of 总结############################
...
## add by one 
li[-2:-1] # last second num  
li[-1:] or li[-1]#the last when the li is [],li[-1]will give error 
li[::5] #每五个取一个。
